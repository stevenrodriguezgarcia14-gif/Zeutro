  ...
  return (
    <group ref={spin} rotation={[-0.16, 0, 0]}>
      <mesh rotation={[Math.PI / 2, 0, 0]} castShadow>
        ...
        <meshPhysicalMaterial attach="material-0" color={m.color} metalness={1} roughness={m.rough} envMapIntensity={2} clearcoat={0.4} clearcoatRoughness={0.3} />
        <meshStandardMaterial attach="material-1" map={front} metalness={0.35} roughness={0.5} envMapIntensity={0.9} />
        <meshStandardMaterial attach="material-2" map={back} metalness={0.35} roughness={0.5} envMapIntensity={0.9} />
      </mesh>
    </group>
  );
}
...
export function Coin3DGL({ tier, glyph, size = 220 }: { tier: Tier; glyph: GlyphKey; size?: number }) {
  ...
  return (
    <div style={{ width: size, height: size }}>
      <Canvas dpr={[1, 1.5]} camera={{ position: [0, 0, 4.3], fov: 32 }} gl={{ antialias: true, alpha: true, powerPreference: "high-performance" }} style={{ background: "transparent" }}>
        <ambientLight intensity={1} />
        <directionalLight position={[3, 4, 5]} intensity={2} />
        <directionalLight position={[-4, -2, 2]} intensity={0.8} color="#bcd3ff" />
