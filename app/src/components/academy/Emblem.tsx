...
export const EMBLEM_TIERS: Record<Tier, TierCfg> = {
  bronce: {
...
export function tierGlow(tier: Tier) { return EMBLEM_TIERS[tier].glow; }

/** Cara del medallón (sin animación de flotación ni candado). Reutilizable en la moneda 3D. */
export function EmblemFace({ tier, glyph, unlocked, size = 84, spin = true }: { tier: Tier; glyph: GlyphKey; unlocked: boolean; size?: number; spin?: boolean }) {
  ...
  return (
    ...
    >
      ...
      >
        {unlocked && spin && (
          <div className="zentro-spin pointer-events-none absolute inset-0"
            style={{ background: "conic-gradient(from 0deg, transparent 0deg, rgba(255,255,255,0.38) 38deg, transparent 92deg, transparent 230deg, rgba(255,255,255,0.14) 268deg, transparent 320deg)", mixBlendMode: "screen" }} />
        )}
        ...
        <svg width={glyphSize} height={glyphSize} viewBox="0 0 24 24" fill="none"
          stroke={glyphColor} strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" className="relative"
...
export function Emblem({
  ...
  return (
    ...
    >
      <EmblemFace tier={tier} glyph={glyph} unlocked={unlocked} size={size} />
      {!unlocked && (
