    ...
    <form action={action} className="space-y-6">
      ...
      <div className="overflow-x-auto rounded-2xl border border-slate-200 bg-white">
              ...
              return (
                <tr key={l.key}>
                  ...
                  <td className="px-3 py-2 text-right">
                    <button type="button" onClick={() => removeLine(l.key)} className="text-slate-400 hover:text-red-600" aria-label="Quitar">✕</button>
                  </td>
                </tr>
              );
            })}
        ...
        <div className="border-t border-slate-100 p-3">
          <button type="button" onClick={addLine} className="rounded-lg border border-slate-300 px-3 py-1.5 text-sm font-medium text-slate-700 hover:bg-slate-50">+ Agregar línea</button>
        </div>
      </div>
